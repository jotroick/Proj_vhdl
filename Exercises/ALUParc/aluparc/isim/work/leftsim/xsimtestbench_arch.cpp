#include "isim/work/leftsim/testbench_arch.h"
...
#include "work/leftshifter/leftshifter.h"
static HSim__s6* IF20(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new workMleftshifter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("X_BUF", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("X_BUFGMUX", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("X_FF", (HSimInstFactoryPtr)IF3);
        (*cfgvh).addVlogModule("X_FF", (HSimInstFactoryPtr)IF4);
        (*cfgvh).addVlogModule("X_INV", (HSimInstFactoryPtr)IF5);
        (*cfgvh).addVlogModule("X_IPAD", (HSimInstFactoryPtr)IF6);
        (*cfgvh).addVlogModule("X_OBUF", (HSimInstFactoryPtr)IF7);
        (*cfgvh).addVlogModule("X_ONE", (HSimInstFactoryPtr)IF8);
        (*cfgvh).addVlogModule("X_OPAD", (HSimInstFactoryPtr)IF9);
        (*cfgvh).addVlogModule("X_ZERO", (HSimInstFactoryPtr)IF10);
        (*cfgvh).addVlogModule("X_ZERO", (HSimInstFactoryPtr)IF11);
        (*cfgvh).addVlogModule("X_OPAD", (HSimInstFactoryPtr)IF12);
        (*cfgvh).addVlogModule("X_ONE", (HSimInstFactoryPtr)IF13);
        (*cfgvh).addVlogModule("X_OBUF", (HSimInstFactoryPtr)IF14);
        (*cfgvh).addVlogModule("X_IPAD", (HSimInstFactoryPtr)IF15);
        (*cfgvh).addVlogModule("X_INV", (HSimInstFactoryPtr)IF16);
        (*cfgvh).addVlogModule("X_FF", (HSimInstFactoryPtr)IF17);
        (*cfgvh).addVlogModule("X_BUFGMUX", (HSimInstFactoryPtr)IF18);
        (*cfgvh).addVlogModule("X_BUF", (HSimInstFactoryPtr)IF19);
        (*cfgvh).addVlogModule("leftshifter", (HSimInstFactoryPtr)IF20);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF21);
        HSim__s6 * topvh = 0;
        topvh = new Work_leftsim_testbench_arch("leftsim");
        addChild(topvh);
        HSim__s5 * topvl = 0;
