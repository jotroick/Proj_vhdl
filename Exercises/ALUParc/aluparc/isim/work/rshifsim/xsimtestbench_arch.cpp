#include "isim/work/rshifsim/testbench_arch.h"
...
#include "simprim_ver.auxlib/_x___a_n_d2/_x___a_n_d2.h"
static HSim__s6* IF2(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new simprim_ver_auxlibM_x___a_n_d2(label); 
    return blk;
}
...
#include "simprim_ver.auxlib/_x___x_o_r2/_x___x_o_r2.h"
static HSim__s6* IF15(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new simprim_ver_auxlibM_x___x_o_r2(label); 
    return blk;
}
...
#include "work/rightshifter/rightshifter.h"
static HSim__s6* IF30(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new workMrightshifter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("X_AND2", (HSimInstFactoryPtr)IF2);
        ...
        (*cfgvh).addVlogModule("X_XOR2", (HSimInstFactoryPtr)IF15);
        (*cfgvh).addVlogModule("X_ZERO", (HSimInstFactoryPtr)IF16);
        (*cfgvh).addVlogModule("X_ZERO", (HSimInstFactoryPtr)IF17);
        (*cfgvh).addVlogModule("X_XOR2", (HSimInstFactoryPtr)IF18);
        (*cfgvh).addVlogModule("X_OPAD", (HSimInstFactoryPtr)IF19);
        (*cfgvh).addVlogModule("X_ONE", (HSimInstFactoryPtr)IF20);
        (*cfgvh).addVlogModule("X_OBUF", (HSimInstFactoryPtr)IF21);
        (*cfgvh).addVlogModule("X_MUX2", (HSimInstFactoryPtr)IF22);
        (*cfgvh).addVlogModule("X_LUT4", (HSimInstFactoryPtr)IF23);
        (*cfgvh).addVlogModule("X_IPAD", (HSimInstFactoryPtr)IF24);
        (*cfgvh).addVlogModule("X_INV", (HSimInstFactoryPtr)IF25);
        (*cfgvh).addVlogModule("X_FF", (HSimInstFactoryPtr)IF26);
        (*cfgvh).addVlogModule("X_BUFGMUX", (HSimInstFactoryPtr)IF27);
        (*cfgvh).addVlogModule("X_BUF", (HSimInstFactoryPtr)IF28);
        (*cfgvh).addVlogModule("X_AND2", (HSimInstFactoryPtr)IF29);
        (*cfgvh).addVlogModule("rightshifter", (HSimInstFactoryPtr)IF30);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF31);
        HSim__s6 * topvh = 0;
        topvh = new Work_rshifsim_testbench_arch("rshifsim");
        addChild(topvh);
        HSim__s5 * topvl = 0;
